...
public class MyTree {
    ...
    private Node deleteRecursive(Node T, Node node) {
        if (T == null) return null;

        int cmp = compare(node.data, T.data);

        if (cmp < 0) {
            T.LC = deleteRecursive(T.LC, node);
        } else if (cmp > 0) {
            T.RC = deleteRecursive(T.RC, node);
        } else if (node.data.getPlayerID() != T.data.getPlayerID()) {
            if (node.data.getPlayerID() < T.data.getPlayerID())
                T.LC = deleteRecursive(T.LC, node);
            else
                T.RC = deleteRecursive(T.RC, node);
        } else {
            ...
            T.data = minNode.data;
            T.RC = deleteRecursive(T.RC, minNode);
        }
        return T;
    }
